...
import { Budget, ErrorResponseSchema, PasswordValidationSchema, SuccessScheme } from "@/src/schemas"
...
export async function deleteBudget(budgetId: Budget['id'], prevState: ActionStateType, formData: FormData) {
    ...
    const currentPassword = PasswordValidationSchema.safeParse(formData.get('currentPassword'))
    ...
    //Comprorbar que el password es correcto
    ...
    const success = SuccessScheme.parse(deleteBudgetJson)
    return {
        errors: [],
        success
    }
